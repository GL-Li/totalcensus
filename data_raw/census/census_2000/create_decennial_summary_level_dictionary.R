...
state <- fread("data_raw/census/census_2000/state_summary_level.txt",
                           header = FALSE, sep = "\n") %>%
    .[, .(code = str_sub(V1, 1, 3),
...
national <- fread("data_raw/census/census_2000/national_summary_level.txt",
                              header = FALSE, sep = "\n") %>%
    .[, .(code = str_sub(V1, 1, 3),
          summary_level = str_sub(V1, 5, nchar(V1)))] %>%
    .[, in_US_file := "yes"] %>%
...
dict_decennial_summarylevel_2000 <- merge(state, national, all = TRUE) %>%
    .[is.na(in_state_file), in_state_file := "-"] %>%
    .[is.na(in_US_file), in_US_file := "-"]


save(dict_decennial_summarylevel_2000, file = "data/dict_decennial_summarylevel_2000.RData")
