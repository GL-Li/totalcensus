...
geo_dict <- read.csv("data_raw/geographic_header_dictionary.csv", sep = ",",
                     stringsAsFactors = FALSE) %>%
    setDT() %>%
    .[, end := start + size -1] %>%
    .[, size := NULL] %>%
    .[, reference := trimws(reference)] %>%
    setcolorder(c("reference", "start", "end", "description"))

save(geo_dict, file = "data/geo_header_dictionary.RData")
