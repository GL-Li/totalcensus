...
Here is the code that gives what we want. The first time you use `read_xxx` functions to read data files, you will be asked to download summary files required for this function call, in this case, 2016 ACS 1-year summary files. Choose 1 to continue. While you can download data during calling the `read_xxx` function, I recommend downloading all data you want with `download_census` function beforehand. The downloading takes time, which can be done when you are sleeping. The details is discussed in section of [Downloading data](#download).
