...
The downloaded data will alse be extracted automatically to the folder `my_census_data`. It takes a long time to download and extract. You can stop downloading any time and then resume downloading by running the function again. 
...
## Basic application
The package has three functions to read decennial census, ACS 5-year survey, and ACS 1-year survey: `read_decennial()`, `read_acs5year()`, and `read_acs1year()`. They are similar but as these  datasets are so different, we prefer to keep three seperate functions, one for each. 
...
- states: the states of which you want read the geograhpy and data files. There are 50 states, "DC" and "PR" (Peuter Rico), plus a special one "US" for natioanal files. You must specify this parameter.
...
- areas: if you know which metropolitan areas, counties, cities and towns you want to get data from, you can specify them here by name or FIPS code, for example, c("New Yok metro", "PLACE = UT62360", "Salt Lake City city, UT").
...
We first need to determing which survey to read. For most recent survey data, we want to read 2016 ACS 1-year surveys, which provide data for geographic areas of population over 65000.
