...
## Install package and download census data
...
This package requires downloading census data to your local computer. You can choose to download only the data you want. For example, if you care only the 2015 ACS 5-year survey data in the states of Massachusette and New Hampshire, you can just download data of these two states, which takes a couple of GB disc space. Or you can download data of all most recent decennial Census, most recent ACS 5-year survey, and most recent ACS 1-year surveys. You need about 200 GB free disc space to download them. Do not worry we have functions to facilitate the downloading. Just follow the steps below.
...
**Step 2**: download and extract census data. It also downloads data generated from Census 2010, which is used to fill missing geographic header records in ACS data. As shown in the examples below, you can choose to download only what you need. If you have more than 200GB free hard drive space, I recommend downloading all recent decennial and ACS data when you are sleeping. A lot of fun to play with these data. Re-run the function to repair possible mistakes during download and extraction.

```{r eval = FALSE}
# download 2015 ACS 5-year survey data of MA and NH, a couple of GB
download_census(survey = "acs5year", year = 2015, states = c("MA", "NH"))

# download 2015 ACS 5-year survey of all states, ~52 GB
...
# download the default datasets, ~200G in total
download_census()
```

You can also download needed data when you run `read_xxx` functions. You will be asked if you want to download data.

The downloaded data will be extracted automatically to the folder `my_census_data`. It takes a long time to download and extract. You can stop downloading any time and then resume downloading by running the function again. 

The data can be found on Census Bureau's website but you do not need to download them manually. Use the function above.
